...
> **Зачем:** консенсус ≠ реализация. Архитектурное обсуждение может закончиться «зафиксировать и думать дальше». Реализуемое решение требует pipeline'а, иначе остаётся inventory (Throughput vs Inventory, см. `memory/distinctions.md`).

Извлечь резюме консенсуса (`grep "^CONSENSUS:" "$PEER_FILE" | sed 's/^CONSENSUS: //'`).

**Спросить пилота явно:**
...
Что дальше?
  ...
  Б. Реализовать → ревью → проверить → задеплоить → Шаг 4
```
