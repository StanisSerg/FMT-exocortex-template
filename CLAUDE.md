...
4. **Pull-on-Touch:** `git pull --rebase` при первом обращении к репо за сессию (все `/home/asus/IWE/*`). Dirty → stash; конфликт → `memory/reference/agent-core.md`.
...
Рабочая директория: `/home/asus/IWE/`; `memory/` = симлинк на auto-memory.
...
Hot-каркас ≤20K токенов (M1), строгая цель ≤12K (M2). Изменил файл из `hot-files.list` (оба CLAUDE.md, rules/*.md) → перед коммитом `/home/asus/IWE/FMT-exocortex-template/scripts/verify-context-budget.sh`.
...
- **Комментарии кода — только EN (решение Андрея, 14.06.2026):** весь `/home/asus/IWE/**`; исключение — user-facing строки по языку интерфейса.
...
- **Именование:** governance-репо называется `DS-strategy` по умолчанию; рабочая директория `/home/asus/IWE/`.
