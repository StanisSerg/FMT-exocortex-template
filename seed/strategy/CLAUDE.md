...
Каждый репозиторий в `/mnt/d/` содержит `WORKPLAN.md` в корне с текущими РП.

**Агрегация:** При создании плана дня/недели Стратег:
1. Обходит все `/mnt/d/*/WORKPLAN.md`
...
```bash
for repo in $(ls /mnt/d/); do
  if [ -d /mnt/d/$repo/.git ]; then
    echo "=== $repo ===" && cd /mnt/d/$repo && git log --oneline --since="1 week ago" 2>/dev/null
  fi
done
```
...
> **Полное описание:** `/mnt/d/CLAUDE.md` секция 2.
